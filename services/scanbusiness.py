...
class ScanBusiness(IBusiness):
    ...
    def __init__(self, service_, rpcmanager_, settings):
        ...
        for s in self.setting['services']:
            if s['enable']:
                self.rpcs[s['coin']] = self.rpcmanager.get_available_feed(s['rpc'])
                self.rpc_tokens[s['coin']]=s['minconf']
    ...
    def process_scan(self): 
        results = db.session.query(Result).filter_by(is_confirm=process.PROCESS_UNCONFIRM).all()
        if not results:
            return True
